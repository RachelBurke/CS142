...
float customer(int hash_inv, int & hash_sold, int coke_inv, int & coke_sold, int mache_inv, int & mache_sold, float & bill) {
	...
	int hash_remain= hash_inv - hash_sold;
	int coke_remain = coke_inv - coke_sold;
	int mache_remain = mache_inv - mache_sold;
	
	cout << "How many hashbrowns?" << endl;
	cin >> hash_sold;
	
	
	while (hash_sold>hash_remain){
		if (hash_sold>hash_remain || hash_sold < 0) {
	...
	cin >> coke_sold;
	while (coke_sold>coke_remain)	{
		if (coke_sold>coke_remain ||  hash_sold < 0) {
	...
	cin >> mache_sold;
	while (mache_sold>mache_remain){
		if (mache_sold>mache_remain || mache_sold < 0) {
...
void bum(int hash_inv, int & hash_sold, int coke_inv, int & coke_sold, int mache_inv, int & mache_sold){
	...
	int new_hash_sold = 3 + rand() % (17-3+1);
	if (new_hash_sold>hash_remain || (hash_remain - new_hash_sold) <0){
		hash_sold += hash_remain;
	}
	else {
	hash_sold = orig_hash_sold + new_hash_sold;
	hash_inv -= hash_sold;
	}
	
	int new_coke_sold = 2 + rand() % (9-2+1);
	if (new_coke_sold>coke_remain || (coke_remain - new_coke_sold) <0){
		coke_sold += coke_remain;
	}
	else {
	coke_sold = orig_coke_sold + new_coke_sold;
	coke_inv -= coke_sold;
	}

	int new_mache_sold = 4 + rand() % (7-4+1);
	if (new_mache_sold>mache_remain || (mache_remain - new_mache_sold) <0){
		mache_sold += mache_remain;
	}
	else {
	mache_sold = orig_mache_sold + new_mache_sold;
	mache_inv -= mache_sold;
	}
}